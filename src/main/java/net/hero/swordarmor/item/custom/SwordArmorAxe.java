...
public class SwordArmorAxe extends AxeItem {
    ...
    private void evaluateAxeEffects(PlayerEntity player) {
        for (Map.Entry<ToolMaterial, StatusEffectInstance> entry : MATERIAL_TO_EFFECT_MAP.entrySet()) {
            ...
            if (hasCorrectAxeOn(mapToolMaterial, player)) {
    ...
    private void addStatusEffectForMaterial(PlayerEntity player, ToolMaterial mapToolMaterial, StatusEffectInstance mapStatusEffect) {
        ...
        if (hasCorrectAxeOn(mapToolMaterial, player) && !hasPlayerEffect) {
            player.addStatusEffect(new StatusEffectInstance(mapStatusEffect.getEffectType(),
                    mapStatusEffect.getDuration(), mapStatusEffect.getAmplifier()));
        }
    }

    private boolean axeHeld(PlayerEntity player) {
        ItemStack hand = player.getInventory().getMainHandStack();

        return !hand.isEmpty();
    }

    private boolean hasCorrectAxeOn(ToolMaterial material, PlayerEntity player) {
        String test = (player.getInventory().getMainHandStack().getItem().toString());
        if (test.contains("axe")) {
